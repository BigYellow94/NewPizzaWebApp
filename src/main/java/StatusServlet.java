...
public class StatusServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (selectedOrders != null && newStatus != null && !newStatus.isEmpty() && newStatus.length() >= 5) {
            try (Connection connection = MySqlConnector.getConnection()) {
                ...
                e.printStackTrace();
                response.getWriter().println("Error occurred while updating status.");
            }
        } else {
            response.getWriter().println("Error: required parameters are not passed or new status is invalid.");
        }
    }
}
